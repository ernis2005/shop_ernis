...
function Puma({ puma }) {
  ...
  if (!data) {
    return (
      <>
        <div className={s.loader}>ccdg</div>{" "}
      </>
    );
  }
  console.log(data);
  let fg = () => {
    const numAscending = [...data].sort(
      (a, b) => a.attributes.price_1 - b.attributes.price_1
    );
    setData(numAscending);
  };
  let fb = () => {
    const numAscending = [...data].sort(
      (a, b) => b.attributes.price_1 - a.attributes.price_1
    );
    setData(numAscending);
  };
  let fga = () => {
  ...
  let fgab = () => {
    const strDescending = [...data].sort((a, b) => (a.name > b.name ? -1 : 1));
    setData(strDescending);
  };
  const Usernnn = (name) => {
    console.log(name);
  ...
  return (
    <>
      ...
      <div className={s.sort}>
        <div>
          <button title="цена/A" onClick={() => fg()}>
            цена/A{" "}
          </button>
          <button title="цена/Z" onClick={() => fb()}>
            цена/Z
          </button>
          <button title="A/Z" onClick={() => fga()}>
            A/Z
          </button>
          <button title="Z/A" onClick={() => fgab()}>
            Z/A
          </button>
        </div>
            ...
            renderOption={(props, data) => (
              <Link href={`/Puma/${data.id}`}>
                <Box component="li" {...props}>
                  <div>
                    <h1 className={s.Autocomplete_h1}>
        ...
        <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8 border-spacing-20">
          ...
          <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
            {data
              .filter(
                (data) =>
                  data.attributes.price_1 > lowerLimit &&
                  data.attributes.price_1 < upperLimit
              )
              .map((product) => (
...
export default Puma;
//https://strapi-production-f3be.up.railway.app/api/pumas?populate=*
